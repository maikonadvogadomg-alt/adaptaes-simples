...
// âââ TTS (Web Speech API) âââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function splitIntoChunks(text: string): string[] {
  const clean = text
    ...
    .slice(0, 1500);
  const parts: string[] = [];
  const sentences = clean.match(/[^.!?â¦]+[.!?â¦]+|[^.!?â¦]{20,}/g) || [clean];
  let cur = "";
...
// âââ Render message content âââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function Chat({ onSendToSearch, pendingMessage, onPendingConsumed }: ChatProps) {
  ...
  const exportChat = () => {
    ...
    const lines = ["=== SK Assistente â Conversa ===", `Data: ${d}`, ""];
    messages.forEach(m => { lines.push(`[${m.role === "user" ? "VOCÃ" : `IA Â· ${m.slotLabel || "Slot"}`}]`); lines.push(m.content); lines.push(""); });
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      {/* ââ Slot selector âââââââââââââââââââââââââââââââââââââââ */}
      ...
      {/* ââ Config panel ââââââââââââââââââââââââââââââââââââââââ */}
      {showConfig && (
        <div className="mx-2 mb-1 p-3 rounded-2xl bg-[#1c2714] border border-gray-700/40 shrink-0 space-y-2">
          ...
          {slots[configSlot]?.key && detectProvider(slots[configSlot].key) && (
            <p className="text-[10px] text-green-400 flex items-center gap-1">
              â Detectado: <strong>{detectProvider(slots[configSlot].key)?.name}</strong>
              &nbsp;Â·&nbsp;{slots[configSlot].model}
            </p>
          )}

          <p className="text-[9px] text-gray-700 mt-1">
            Prefixos: gsk_ = Groq Â· sk- = OpenAI Â· AIza = Google Â· pplx- = Perplexity Â· xai- = Grok Â· sk-ant = Anthropic Â· sk-or- = OpenRouter
          </p>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2 min-h-0">
        {allMsgs.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center gap-3 text-center py-12">
            <div className="w-16 h-16 rounded-full bg-green-900/30 border border-green-700/30 flex items-center justify-center text-3xl select-none">ð¿</div>
            <div>
              ...
              <p className="text-gray-600 text-[11px] mt-1 leading-relaxed">
                ...
                VocÃª pode usar Groq (gratuito), OpenAI, Gemini,<br/>
                Perplexity, xAI ou OpenRouter.
              </p>
            </div>
            <div className="flex flex-wrap gap-1.5 justify-center mt-1">
              {["Me ajude a planejar algo", "Analise este texto:", "Resuma isso para mim:", "O que vocÃª consegue fazer?"].map(s => (
                <button
                  key={s}
                  onClick={() => setPrompt(s)}
                ...
                <>
                  ...
                  {(msg as any).isStreaming ? (
                    ...
                    <div className="flex items-center gap-2.5 mt-2 pt-1.5 border-t border-gray-700/20">
                      ...
                      {onSendToSearch && (
                        <button
                          onClick={() => onSendToSearch(msg.content.slice(0, 100).replace(/[^\w\sÃ¡Ã©Ã­Ã³ÃºÃ£ÃµÃ¢ÃªÃ´Ã Ã¼Ã§]/g, " ").trim())}
      ...
      <div className="border-t border-gray-700/40 bg-[#1c2714] px-2 pt-1.5 pb-2 shrink-0">
        <div className="flex items-center gap-1 mb-1.5">
          ...
          <button
            ...
            title={ttsEnabled ? "Voz ativada â clique para desativar" : "Voz desativada â clique para ativar"}
        ...
        <div className="flex gap-1.5 items-end">
          <textarea
            ...
            placeholder="Digite ou fale sua mensagemâ¦ (Enter envia Â· Shift+Enter nova linha)"
            rows={1}
