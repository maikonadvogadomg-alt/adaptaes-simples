...
// Caminho real do binÃ¡rio `script` (detectado em runtime, nÃ£o hardcoded)
let SCRIPT_BIN = "/usr/bin/script";

// PATH expandido para garantir acesso a npm, node, python, etc. em produÃ§Ã£o
const NIX_PATHS = [
...
const FULL_PATH = [...NIX_PATHS, ...(process.env["PATH"] ?? "").split(":")].filter(Boolean).join(":");
...
function detectTermMode(): TermMode {
  ...
  const scriptCheck = spawnSync("which", ["script"], {
    stdio: "pipe", encoding: "utf8",
    env: { ...process.env, PATH: FULL_PATH },
  });
  if (scriptCheck.status === 0 && scriptCheck.stdout.trim()) {
    SCRIPT_BIN = scriptCheck.stdout.trim(); // usa o caminho real, nÃ£o hardcoded
    logger.info({ path: SCRIPT_BIN }, "Terminal PTY via script command");
    return "script";
  }
...
wss.on("connection", (ws: WebSocket) => {
  ...
  if (TERM_MODE === "pty_helper") {
    ...
    shellCmd = SCRIPT_BIN; // usa o caminho detectado em runtime (nÃ£o hardcoded)
